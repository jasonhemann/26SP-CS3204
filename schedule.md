 ...
 <summary><strong>{{ week_start_seconds | date: "%b-%d" }} - {{ week_end_seconds | date: "%b-%d" }} </strong></summary>
  ...
  <li><strong>Assignments:</strong>
	<ul>
	  ...
	  <li><strong>{{ hw.title }}:</strong> Assigned on {{ out_day_seconds | date: '%a, %b %d' }} {% if hw.starter_code %} | <a href="{{ site.sourceurl }}{{ site.repository }}/tree/master/_starter_code/{{ hw.starter_code }}">Assignment</a>{% endif %}</li>
	  {% endfor %}
	</ul>
 </li>
